# App Store listing — socloudy

## Name
socloudy

## Subtitle (≤30 chars)
- EN: `Cute, cloudy Bluesky client`
- JA: `かわいい雲の Bluesky`

## Promotional text (≤170 chars)
- EN: `A lightweight, cute, quirky Bluesky client — your timeline as fluffy clouds drifting across a blue sky.`
- JA: `軽量でかわいい、ちょっと風変わりな Bluesky クライアント。タイムラインを青空に浮かぶ雲のカードで。`

## Keywords (≤100 chars, comma-separated, per locale)
- EN: `bluesky,atproto,social,sns,bluesky client,timeline,feeds,microblog,cloud`
- JA: `ブルースカイ,bluesky,atproto,SNS,クライアント,雲,タイムライン,フィード`

## Description

### EN
socloudy is a lightweight, cute, and slightly quirky Bluesky client.

Posts appear as fluffy "cloud" cards drifting across a blue sky — a playful
design where the cards move as you scroll.

* socloudy is an independent, third-party client for Bluesky (the AT Protocol)
and is not affiliated with Bluesky.

### JA
socloudy は、軽量でかわいい、ちょっと風変わりな Bluesky クライアントです。

ポストを青空にふわふわ浮かぶ「雲」のカードとして表示。
スクロールに合わせてカードが動く、遊び心のあるデザインです。

* socloudy は Bluesky（AT Protocol）の独立したサードパーティ製クライアントであり、Bluesky 公式とは関係ありません。

## Copyright (App Store Connect field)
`2026 So Asano`
(App Store displays it as "© 2026 So Asano".)

## Support / Marketing URL
https://socloudy.app

## Notes
- Screenshots: store/screenshot-1.png … 3 (1290×2796, 6.7"). Drop the actual app
  screenshot into the dashed placeholder, then upload.
  1. Sign in with your Bluesky account
  2. Cute, cloudy design
  3. Dark mode
- A privacy policy URL is required (host one on socloudy.app).
- UGC apps (Guideline 1.2) need report / block / mute + an EULA before review.
